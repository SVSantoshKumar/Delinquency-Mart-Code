CREATE OR REPLACE PROCEDURE DISHA_L2_CURATED.BASE_HARDCODE.RUN_EXCEL_QUERIES_DYNAMIC1("META_TABLE_NAME" VARCHAR, "START_DATE" DATE DEFAULT DATE_ADDMONTHSTODATE(-1, CURRENT_DATE()))
RETURNS VARCHAR
LANGUAGE JAVASCRIPT
EXECUTE AS CALLER
AS '
...
let metaColsSQL = `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=''${META_TABLE_NAME.split(''.'').pop()}'' AND TABLE_SCHEMA=''${META_TABLE_NAME.split(''.'')[META_TABLE_NAME.split(''.'').length-2]}''`;
...
while(cur.next()){
    let serial_no = cur.getColumnValue(''Serial No'');
    let query_template = cur.getColumnValue(''SQL Query'');
...
snowflake.createStatement({sqlText:errorSQL,binds:[ERROR_MSG,FAILED_Q].concat(metaValues)}).execute();
    ...
    try{
        ...
        let metaColNamesSQL = metaCols.map(c=>`\\"${c}\\"`).join('', '');
        let metaPlaceholders = metaCols.map(()=>''?'').join('', '');
        let insertSQL = `INSERT INTO ${EXECUTION_LOG_TABLE}(SERIAL_NO,EXECUTED_ON,EXECUTED_QUERY,REPORT_DATE_USED,QUERY_OUTPUT,RUN_ID,${metaColNamesSQL}) SELECT ?,CURRENT_TIMESTAMP,?,?,PARSE_JSON(?),?,${metaPlaceholders}`;
        let historySQL = `INSERT INTO ${EXECUTION_HISTORY_TABLE}(SERIAL_NO,EXECUTED_ON,EXECUTED_QUERY,REPORT_DATE_USED,QUERY_OUTPUT,RUN_ID,${metaColNamesSQL}) SELECT ?,CURRENT_TIMESTAMP,?,?,PARSE_JSON(?),?,${metaPlaceholders}`;
        if(!result.next()){
            ...
            do{
                ...
                let insertSQLDynamic = `INSERT INTO ${EXECUTION_LOG_TABLE}(SERIAL_NO,EXECUTED_ON,EXECUTED_QUERY,REPORT_DATE_USED,RUN_ID,${metaColNamesSQL},${colNamesSQL}) SELECT ?,CURRENT_TIMESTAMP,?,?,?,${metaPlaceholders},${colPlaceholders}`;
                let historySQLDynamic = `INSERT INTO ${EXECUTION_HISTORY_TABLE}(SERIAL_NO,EXECUTED_ON,EXECUTED_QUERY,REPORT_DATE_USED,RUN_ID,${metaColNamesSQL},${colNamesSQL}) SELECT ?,CURRENT_TIMESTAMP,?,?,?,${metaPlaceholders},${colPlaceholders}`;
